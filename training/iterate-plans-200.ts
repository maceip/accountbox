...
import { generate, loadBaseModel, equipAdapter, buildGmailTrainingExamples } from "../src/lib/runtime/accountbox-runtime";
...
function improveTarget(p: any): any {
  ...
  if (prompt.includes("draft") && prompt.includes("reply") && plan.steps && plan.steps.length === 2 && plan.steps[1].tool === "create_draft") {
    // already good; optionally insert read if not present
    if (plan.steps[0].tool === "search_messages" && plan.steps[1].tool !== "read_message") {
...
async function main() {
  console.log("=== Starting 200-iteration loop (ask the current fine-tuned targets the 18 prompts repeatedly) ===");
  ...
  while (totalAsks < maxAsks) {
    ...
    for (let i = 0; i < prompts.length; i++) {
      ...
      console.log(`Iter ${totalAsks} (pass ${pass}): ${p.prompt.slice(0,55)}...`);
    ...
    for (const ex of examples) {
      ...
      let pl: any = {}; try { pl = JSON.parse(r); } catch {}
      fullTotal += scorePlan(pl, ex.target);
    }
    const globalAvg = examples.length ? fullTotal / examples.length : 0;
    console.log(`--- pass ${pass} done, asks so far: ${totalAsks}, round avg ${avgThisRound.toFixed(2)}, global avg now ${globalAvg.toFixed(2)}`);

    if (roundCount === 0) break;
  }

  console.log(`\n=== Finished. Total asks: ${totalAsks}. Global history (sample): ${history.slice(0,5).map(x=>x.toFixed(2)).join(", ")} ... ${history.slice(-3).map(x=>x.toFixed(2)).join(", ")}`);
