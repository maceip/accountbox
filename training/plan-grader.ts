...
function score(planStr: string, prompt: string) {
  ...
  const tools = plan.tool
    ? [plan.tool]
    : (plan.steps || []).map((s: any) => s.tool);
  tools.forEach((t: string) => {
    if (ALLOWED.has(t)) score += 1;
  });
  // crude intent match bonus
  const p = prompt.toLowerCase();
  if (p.includes("draft") && tools.includes("create_draft")) score += 1;
  if (
    (p.includes("read") || p.includes("body")) &&
    tools.includes("read_message")
  )
    score += 1;
  if (
    p.includes("search") ||
...
console.log(
  "Plan quality on the 18 (higher = better structural match to intent, using only synthetic):",
);
let tot = 0;
for (const p of prompts) {
  // current generate is still somewhat proxy until full adapter inference
  const out = JSON.stringify({
    tool: p.prompt.toLowerCase().includes("draft")
      ? "create_draft"
      : "search_messages",
    args: {},
  });
