...
const SENDER_QUERY: Record<string, string> = {
  "my manager": "from:manager",
  Sarah: "from:sarah",
  "the finance team": "from:finance",
  legal: "from:legal",
  "the design lead": "from:design",
  recruiting: "from:recruiting",
  "IT support": "from:it",
  "the vendor": "from:vendor",
  Alex: "from:alex",
  "the CEO": "from:ceo",
  billing: "from:billing",
  "the security team": "from:security",
};
const TOPICS = [
  "the product launch",
  "Q3 planning",
...
const TOPIC_KW: Record<string, string> = {
  "the product launch": '"product launch"',
  "Q3 planning": '"Q3 planning"',
  "the offsite": "offsite",
  "invoice approval": "invoice",
  "the incident postmortem": "postmortem",
  "contract renewal": '"contract renewal"',
  "the design review": '"design review"',
...
const WINDOWS: Array<[string, string]> = [
  ...
  ["today", "newer_than:1d"],
  ["the last two weeks", "newer_than:14d"],
...
for (let i = 0; i < 46; i++) {
  const s = pick(SENDERS),
    t = pick(TOPICS),
    [wText, wQ] = pick(WINDOWS),
    label = pick(LABELS);
  const variant = i % 8;
  if (variant === 0)
    add(
      `Find all unread emails from ${s} ${wText} that mention ${t}.`,
      {
        tool: "search_messages",
        args: { query: `${SENDER_QUERY[s]} ${TOPIC_KW[t]} is:unread ${wQ}` },
      },
      ["search_messages"],
    );
  else if (variant === 1)
    add(
      `Show me everything with the '${label}' label from ${wText}.`,
      { tool: "search_messages", args: { query: `label:${label} ${wQ}` } },
      ["search_messages"],
    );
  else if (variant === 2)
    add(
      `List my starred messages about ${t}.`,
      { tool: "search_messages", args: { query: `is:starred ${TOPIC_KW[t]}` } },
      ["search_messages"],
    );
  else if (variant === 3)
    add(
      `Search for attachments related to ${t} from ${wText}.`,
      {
        tool: "search_messages",
        args: { query: `has:attachment ${TOPIC_KW[t]} ${wQ}` },
      },
      ["search_messages"],
    );
  else if (variant === 4)
    add(
      `What did ${s} send me ${wText}?`,
      { tool: "search_messages", args: { query: `${SENDER_QUERY[s]} ${wQ}` } },
      ["search_messages"],
    );
  else if (variant === 5)
    add(
      `Do I have any unread mail from ${s}?`,
      {
        tool: "search_messages",
        args: { query: `${SENDER_QUERY[s]} is:unread` },
      },
      ["search_messages"],
    );
  else if (variant === 6)
    add(
      `Find messages in my sent mail about ${t}.`,
      { tool: "search_messages", args: { query: `in:sent ${TOPIC_KW[t]}` } },
      ["search_messages"],
    );
  else
    add(
      `List emails mentioning ${t} or ${pick(TOPICS)} from ${wText}.`,
      {
        tool: "search_messages",
        args: { query: `(${TOPIC_KW[t]} OR ${TOPIC_KW[pick(TOPICS)]}) ${wQ}` },
      },
      ["search_messages"],
    );
}
...
for (let i = 0; i < 18; i++) {
  const s = pick(SENDERS),
    ...
    [wText, wQ] = pick(WINDOWS);
  const q =
    i % 2 === 0 ? `${SENDER_QUERY[s]} ${TOPIC_KW[t]}` : `${TOPIC_KW[t]} ${wQ}`;
  const phr = i % 3;
  const prompt =
    phr === 0
      ? `Open the latest email from ${s} about ${t} and summarize it.`
      : phr === 1
        ? `Read the most recent message about ${t} from ${wText} and tell me the action items.`
  ...
  add(
    prompt,
    {
      steps: [
        {
          tool: "search_messages",
          args: { query: `${SENDER_QUERY[s]} ${TOPIC_KW[t]}` },
        },
        {
          tool: "create_draft",
          args: {
            ...
            body:
              i % 2 === 0
                ? "Thanks — I'll review this and get back to you by Friday."
                : `Quick follow-up on ${t} — any update on the latest status?`,
          },
        },
      ],
    },
...
for (let i = 0; i < 8; i++) {
  ...
  add(
    `Draft a short note to ${to.split("@")[0]} saying the ${t} work is on track.`,
    {
      tool: "create_draft",
      args: {
        to,
        subject: `${t} update`,
        body: `Quick update: the ${t} work is on track. More details to follow.`,
      },
    },
    ["create_draft"],
  );
...
const out = {
  description:
    "Deterministic balanced expansion (bun run training/expand-gmail-dataset.ts). Search-heavy on purpose: the measured failure of the 19-row adapter was appending create_draft to search-only intents.",
  prompts: rows,
};
writeFileSync(
  "training/expanded-prompts.json",
  `${JSON.stringify(out, null, 2)}\n`,
);
const dist: Record<string, number> = {};
for (const r of rows) {
  const k = r.expected_tools.join("+");
  dist[k] = (dist[k] || 0) + 1;
}
