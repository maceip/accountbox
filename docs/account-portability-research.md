...
Keep everything as-is; fix the _communication_:
...
- Requires a _user-memorable_ identity (email they actually own, or username) instead of the minted `vault-…@vault.localhost` — that's an onboarding change, and account enumeration / rate limiting become real concerns.
...
- WebAuthn passkey with the **PRF extension** derives a stable secret that wraps the vault key. Passkeys sync across the user's devices via iCloud Keychain / Google Password Manager — the _platform_ does the cross-device transport, we never see key material.
...
- Session semantics on shared machines: should Unlock in a _new_ browser invalidate other sessions (Better Auth revocation) or coexist?
