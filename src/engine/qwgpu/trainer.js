...
export class QwenLoraTrainer {
  ...
  /*
   * TECHNIQUE: Per-token FLOAT loss weights (the GRPO hook)
   *   The CE kernel computes dLogits = mask[t] * lossScale * (softmax - onehot).
   *   With mask in {0,1} that's plain SFT cross-entropy. With mask[t] set to a
   *   sequence advantage A (any float, including negative) it becomes the
   *   policy-gradient of -A * log p(token) — GRPO with zero kernel changes.
   *   numActive counts NONZERO weights so lossScale stays a per-token mean.
   */
  _writeTargets(tokens, lossMask, T) {
    ...
    for (let t = 0; t < T - 1; t++) {
      ...
      const mk = lossMask ? Number(lossMask[t]) || 0 : 1;
      mask[t] = mk;
      if (mk !== 0) numActive++;
    }
    targets[T - 1] = 0;
