...
export async function createEmberglassEngine(opts = {}) {
  ...
  return {
    ...
    async chatComplete(messages, { maxTokens = 4096, temperature = 0, topK, topP } = {}) {
      // topK/topP pass through to ModelSession.generate (which already supports
      // them); undefined keeps its defaults, so existing callers are unchanged.
      let out = '';
      for await (const delta of session.generate(messages, { maxTokens, temperature, topK, topP })) out += delta;
      return out;
    },
    dispose() {
      session.rt = null;
