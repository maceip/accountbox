...
import { extractPlanJson, isValidToolPlan, firstValidPlanObject } from "./plan-parse";

// ---- recovers a genuine plan the model produced ----

test("clean single-tool plan", () => {
  const p = extractPlanJson('{"tool":"search_messages","args":{"query":"is:unread"}}');
  expect(isValidToolPlan(p)).toBe(true);
  expect(p.tool).toBe("search_messages");
});

test("clean multi-step plan", () => {
  const p = extractPlanJson('{"steps":[{"tool":"search_messages","args":{"query":"x"}},{"tool":"read_message","args":{"id":"1"}}]}');
  expect(isValidToolPlan(p)).toBe(true);
...
test("recovers correct plan followed by int4 trailing garbage (real failure shape)", () => {
  ...
  const raw = '{"tool":"search_messages","args":{"query":"from:manager launch"}} newer_than:7d"} — "to":"manager"}';
  const p = extractPlanJson(raw);
  expect(isValidToolPlan(p)).toBe(true);
  expect(p.tool).toBe("search_messages");
...
test("braces inside string values do not break extraction", () => {
  const p = extractPlanJson('{"tool":"create_draft","args":{"to":"a@b.com","subject":"{weird}","body":"} } {"}}');
  expect(isValidToolPlan(p)).toBe(true);
...
test("repetition loop is not a valid plan (stays cold)", () => {
  const raw = '{"tool":"search_messages","args":{"query":"label:project-x-x","label":"label:project-x-x","body":"label:project-x-x"';
...
test("smeared / never-closed object is not fabricated into a plan", () => {
  const raw = '{"tool":"create_draft","args":{"to":"all-hands incompany.com","subject":"Re: the meeting","body":"Quick status: all-hands inc';
