...
catch (e) { console.log("kernels: skipped —", String(e).slice(0, 200)); }
...
await build({
  ...
  loader: { ".wgsl": "text" },
  legalComments: "none", keepNames: true, treeShaking: true,
...
try {
  ...
  page.on("console", (m) => {
    const t = m.text();
    if (t.startsWith("GATE ")) { rows.push(t.slice(5)); console.log("GATE>", t.slice(5, 320)); }
    else console.log("  b>", t.slice(0, 160));
  });
  page.on("pageerror", (e) => console.log("PAGEERR", String(e).slice(0, 400)));
  await page.goto(`http://127.0.0.1:${port}/gate.html`, { waitUntil: "domcontentloaded" });
  ...
  const parsed = rows.map((l) => { try { return JSON.parse(l); } catch { return { raw: l }; } });
  ...
  const match = realPlan.filter((p) => p.tools && p.expected &&
    new Set(p.tools).size === new Set(p.expected).size && p.expected.every((t) => p.tools.includes(t)));

  // Show what the model actually said when it didn't produce a plan (honesty).
  for (const p of ranButBad) console.log(`  ran-but-not-a-plan [${p.i}] raw: ${p.raw}`);
  ...
  await writeFile(join(ROOT, "gate-artifact.json"), JSON.stringify(artifact, null, 2));
  console.log("\n=== GATE RESULT ===");
  ...
  const pass = artifact.equipped && !err && plans.length === 18 &&
    trueCold.length === 0 && realPlan.length >= 1;
  if (pass) console.log(`GATE: PASS — real WebGPU inference ran on all ${plans.length} prompts; ${realPlan.length} valid plans, ${ranButBad.length} non-plan outputs (all weight-driven, no replay)`);
  else { console.error("GATE: FAIL — trueCold:", trueCold.length, "validPlan:", realPlan.length); process.exitCode = 1; }
} finally {
  await browser.close();
