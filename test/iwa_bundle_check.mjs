...
const swbn = new Uint8Array(
  readFileSync(join(ROOT, "dist-iwa/accountbox.swbn")),
);
...
check(
  "integrity block parses, bundle ID matches key",
  parsedId === expectedId,
  parsedId,
);
...
for (let i = 2; i < swbn.length - 8; i++) {
  if (emoji.every((b, j) => swbn[i + j] === b) && swbn[i - 1] === 0x48) {
    at = i - 2;
    break;
  }
}
...
const chunkBody = appChunk
  ? Buffer.from(bundle.getResponse(appChunk).body).toString()
  : "";
check(
  "real app code packaged",
  chunkBody.includes("Setup Secure Workspace"),
  appChunk,
);
...
check(
  ...
  !urls.some(
    (u) =>
      u.startsWith("/model/") ||
      u.startsWith("/model-chat/") ||
      u.startsWith("/adapters/"),
  ),
);
...
const wk = JSON.parse(
  Buffer.from(
    bundle.getResponse("/.well-known/manifest.webmanifest").body,
  ).toString(),
);
check(
  "well-known manifest has version",
  typeof wk.version === "string",
  wk.version,
);
