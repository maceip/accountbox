...
try {
  // ---------- desktop 1280x800: fresh vault -> journey ----------
  {
    ...
    const journeyPitch = await page.getByText("You build this workspace yourself.").isVisible();
    step("desktop: journey two-column layout (pitch visible)", journeyPitch);
    const s1 = await page.locator('[data-journey-step="chat-agent"]').getAttribute("data-step-state");
    const s2 = await page.locator('[data-journey-step="first-skill"]').getAttribute("data-step-state");
    step("desktop: step 1 active, step 2 locked", s1 === "active" && s2 === "locked", `${s1}/${s2}`);
    const noSidebar = (await page.locator('[data-slot="sidebar"]').count()) === 0;
    const noConnectTile = (await page.getByRole("button", { name: "Connect Gmail" }).count()) === 0;
    const noChatTile = (await page.getByPlaceholder("e.g. Find all unread from manager this week...").count()) === 0;
    step("desktop: no sidebar/board/agent-tile in journey", noSidebar && noConnectTile && noChatTile);
  ...
  // ---------- phone 390x844: fresh vault -> journey (stacked) ----------
  {
    ...
    const journeyPitchHidden = !(await page.getByText("You build this workspace yourself.").isVisible().catch(() => false));
    step("phone: journey stacked (pitch hidden)", journeyPitchHidden);
    const noLauncher = (await page.getByRole("button", { name: "Local agent" }).count()) === 0;
  ...
  {
    const ctx = await completedContext({ width: 390, height: 844 }, { isMobile: true, hasTouch: true });
    const page = await ctx.newPage();
    await page.goto(`${BASE}/`);
    await page.getByRole("button", { name: "Generate a recovery key" }).waitFor({ timeout: 15_000 });
    await page.getByText("Set my own password instead").click();
    await page.getByPlaceholder("Master password").waitFor({ timeout: 5_000 });
    await createVault(page);
  ...
  // ---------- foldable-ish 840x800: fresh -> journey, done -> board ----------
  {
    ...
    await page2.getByPlaceholder("Master password").waitFor({ timeout: 15_000 });
    await createVault(page2);
    await page2.getByPlaceholder("e.g. Find all unread from manager this week...").waitFor({ timeout: 30_000 });
    const connect = await page2.getByRole("button", { name: "Connect Gmail" }).isVisible();
