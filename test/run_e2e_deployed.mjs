...
async function preChecks() {
  ...
  step("skill model shard supports Range", shard.status === 206, `status=${shard.status}`);

  const chatCfg = await fetch(`${BASE}/model-chat/config.json`);
  step("chat model config served", chatCfg.ok, `status=${chatCfg.status}`);

  const chatShard = await fetch(`${BASE}/model-chat/model-00001-of-00002.safetensors`, {
    headers: { Range: "bytes=0-1023" },
  });
  step("chat model shard supports Range", chatShard.status === 206, `status=${chatShard.status}`);
...
async function main() {
  ...
  page.on("console", (m) => {
    ...
    if (t.includes("[chat-runtime] state -> unloaded") && t.includes("another model took the GPU"))
      chatDisplaced = true;
    ...
    if (t.startsWith("[emberglass") || t.startsWith("[chat-runtime]") || t.startsWith("[agent:")) {
  ...
  try {
    ...
    await page.locator('[data-journey-screen="overview"]').waitFor({ timeout: 60_000 });
    const noSidebar = (await page.locator('[data-slot="sidebar"]').count()) === 0;
    ...
    console.log("  streaming CHAT model over the wire (first load can take many minutes)…");
    ...
    await page.locator("[data-journey-advance]").waitFor({ timeout: 5 * 60_000 });
    const chatReply = await page.locator("div.whitespace-pre-wrap").last().textContent();
    step("real chat exchange completed step 1", true, `reply: ${chatReply.trim().slice(0, 100).replace(/\n/g, " ")}`);
    await page.locator("[data-journey-advance]").click();

    // ---- STEP 2: first skill — manifest picker, engine swap, real plans ----
    const s2 = await page.locator('[data-journey-step="first-skill"]').getAttribute("data-step-state");
    ...
    for (const prompt of SKILL_PROMPTS) {
      ...
      console.log(`  no plan (honest non-plan output) for: ${prompt.slice(0, 60)}…`);
    }
    step("real inference ran for skill prompts", sawRealPath >= 1, `REAL-path calls=${sawRealPath}`);
    step("no true-cold (unequipped) generation", sawTrueCold === 0, `trueCold=${sawTrueCold}`);
    step("weight-driven plan rendered (planned-not-executed)", plansRendered >= 1);
    ...
    const s3 = await page.locator('[data-journey-step="connect-account"]').getAttribute("data-step-state");
    step("overview shows step 3 active", s3 === "active", `state=${s3}`);
    await page.getByRole("button", { name: "Start" }).click();
    await page.locator('[data-journey-screen="connect-account"]').waitFor({ timeout: 15_000 });
    await page.locator('[data-connect-state="waiting"]').waitFor({ timeout: 15_000 });
    step("connect gate renders (journey stops here in CI)", true);
    const stillNoSidebar = (await page.locator('[data-slot="sidebar"]').count()) === 0;
